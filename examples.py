...
entity type name can contains only letters and numbers.
"""
task_id = client.create_entity_type(
    [{"name": "EntityTypeA", "help": "My new entity."}])


"""
Example for update entity type method.
"""
task_id = client.update_entity_type(
    [{"name": "EntityTypeA", "help": "test update description"}])
...
"""
result = client.read("Field", ['id', 'name'], [['name', 'is', 'text'], [
                     'entity_type__name', 'is', 'Task']], pages={"page": 1, "page_size": 1})
...
"""
client.create("Task", data=[{"name": "New Layout", "project": {
              "id": 1, "type": "Project"}, "entity": {"id": 1, "type": "Shot"}}])
...
client.read("Task", filters=[["name", "is", "Layout"], [
            "project", "is", {"id": 2, "type": "Project"}]])


# EXAMPLE: read() with 'or' filters.
client.read("Task", filters=["or", ["name", "is", "Layout"], [
            "project", "is", {"id": 2, "type": "Project"}]])


# EXAMPLE: read() with complex filters.
client.read("Task", filters=[["or", ["name", "is", "Layout"], [
            "project", "is", {"id": 2, "type": "Project"}]], ["status", "is", "wtg"]])


# EXAMPLE: read() with single sorts.
client.read("Task", fields=["id", "name"], sorts=[
            {"column": "name", "direction": "ASC"}])


# EXAMPLE: read() with multiple types of sorts.
client.read("Task", fields=["id", "name", "status"], sorts=[
            {"column": "name", "direction": "ASC"}, {"column": "status", "direction": "DESC"}])


# EXAMPLE: read() with groups.
client.read("Task", fields=["id", "name"], groups=[
            {"column": "name", "method": "exact", "direction": "asc"}])
...
"""
Long polling event log entries.
"""
